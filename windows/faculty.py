...
class Faculty:
    ...
    def __insert_record_student(self):
        query = '''select id,concat_ws(" ",first_name,middle_name,last_name),roll_no,std,division,sex,phoneno,dob from student order by id;'''
    ...
    def __search_engine_for_student(self, column):
        if self.search.get() == "":
            ...
            if column == "sex":
                query = f'''select id,concat_ws(" ",first_name,middle_name,last_name),roll_no,std,division,sex,phoneno,dob from student where {column} like "{self.search.get()}%";'''
            else:
                query = f'''select id,concat_ws(" ",first_name,middle_name,last_name),roll_no,std,division,sex,phoneno,dob from student where {column} like "%{self.search.get()}%";'''
