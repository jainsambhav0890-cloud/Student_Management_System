...
class admin:
    ...
    def add_faculty_into_data(self):
        if self.valid():
            ...
            if backend.call_procedure('addfaculty', val):
                ...
                if backend.execute_query("insert into login_details values (%s,%s,sha2(%s,256),'faculty');", (id, self.uname.get(), self.passw.get())):
    ...
    def update_faculty(self):
        if self.valid():
            ...
            if backend.execute_query(query, val):
                if backend.execute_query('update login_details set uname=%s,password=sha2(%s,256) where id=%s', (self.uname.get(), self.passw.get(), self.id.get())):
    ...
    def __add_student_intodata(self):
        if self.valid1():
            ...
            if backend.call_procedure('addstudent', val):
                ...
                if backend.execute_query("insert into login_details values (%s,%s,sha2(%s,256),'student');", (id, self.uname.get(), self.passw.get())):
    ...
    def update_student(self):
        if self.valid2():
            query = '''update student set first_name=%s,middle_name=%s,last_name=%s,roll_no=%s,division=%s,address=%s,father_name=%s,mother_name=%s,std=%s,dob=%s,bloodgroup=%s,doa=%s,father_occ=%s,mother_occ=%s,phoneno=%s,sex=%s where id=%s'''
            ...
            if backend.execute_query(query, val):
                if backend.execute_query('update login_details set uname=%s,password=sha2(%s,256) where id=%s', (self.username.get(), self.password.get(), self.id1.get())):
